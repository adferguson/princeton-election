...
import sys, urllib2, datetime, time, xml.dom.minidom
from numpy import *
...
def main():
	...
	# Hard-code 2004 result for D.C. since there is no polling data
	# available and Political analysts do not disagree on the expected
	# outcome.
...
def write_statistics(pfile, polls):
	...
	if num == 0:
		...
		pfile.write("%s " % len(last_three))
		pfile.write("%s " % int(last_three[-1][3].strftime("%j")))
		pfile.write("%s %s " % get_statistics(last_three))
